...
class MuZeroConfig:
    def __init__(self, args):
        ...
        self.observation_shape = (1, 1,
                                  128)  # Dimensions of the game observation, must be 3D (channel, height, width). For a 1D array, please reshape it to (1, 1, length of array)
...
class Game(AbstractGame):
    ...
    def __init__(self, seed=None):
        self.env = gym.make("VideoPinball-ram-v0")
        if seed is not None:
            self.env.seed(seed)
    ...
    def reset(self):
        ...
        """
        print(numpy.shape(numpy.array([[self.env.reset()]])))
